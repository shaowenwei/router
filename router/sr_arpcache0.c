...
void SendIcmp(struct sr_instance* sr, sr_icmp_code_t icmpCode, uint8_t* packet)
{
    ...
    uint8_t eshost = ether_hdr->ether_shost;
...
void handle_arpreq(struct sr_arpreq* req, struct sr_instance *sr){
    ...
    if (difftime(curtime, req->sent) >= 1) {
        if(req->times_sent >= 5)
        {
            ...
            {
                SendIcmp(sr, icmp_code_destination_host_unreachable,packet->buf);
            }
        ...
        else
        {
            ...
            sr_if* interface = IpGetAddr(sr, req->ip);
            unsigned char s_addr[ETHER_ADDR_LEN] = interface->addr;
            /* Ethernet Header */
...
void sr_arpcache_sweepreqs(struct sr_instance *sr) {
    /* Fill this in */
    sr_arpreq* request = sr->cache.requests;
    while(request)
    {
