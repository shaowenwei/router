...
static const uint8_t broadcastEthertAddr[ETHER_ADDR_LEN] =
...
int check_mac_addr_equal(const uint8_t *add1, const uint8_t *add2)
{
    for (int i = 0; i < 6; ++i)
    {
        if (*add1 != *add2)
...
void sr_handlepacket(struct sr_instance* sr, uint8_t* packet, unsigned int len, char* interface)
{
    ...
    if(eth_h->ether_type == ethertype_ip)
    {
        ...
        // packet is for me
        if(check_mac_addr_equal(iface->addr, eth_h->ether_shost))
        {
            // icmp packet
            if(ip_h->ip_p == ip_protocol_icmp)
            {
                
                struct sr_icmp_hdr* icmp_echo;
            ...
            {
                struct sr_icmp_hdr* icmp_unreach;
        ...
        else // packet is not for me
        {
            ...
            while(rt_walker != NULL){
                ...
                uint32_t ip_tmp = (rt_walker->mask.s_addr) & ip_dst;
